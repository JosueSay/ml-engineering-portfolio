...
def construir_silver(config: dict | None = None) -> pd.DataFrame:
    ...
    df.to_csv(silver_dir / cfg["archivos"]["silver"], index=False)
...
def construir_gold(
    ...
    if not df_silver.empty:
        ...
        fecha_inicio = fecha_fin - timedelta(weeks=cfg["modelado"]["historia_sintetica_semanas"])
    ...
    gold.to_csv(gold_dir / cfg["archivos"]["gold"], index=False)
...
def cargar_gold_si_existe(config: dict | None = None) -> pd.DataFrame | None:
    ...
    """
    cfg = config or load_config()
    ruta = resolve_path(cfg["paths"]["gold"]) / cfg["archivos"]["gold"]
    if not ruta.exists():
        return None
